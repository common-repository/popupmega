...
class Wprp_Email_Manager {
	...
	function hooks() {
		...
		add_action( 'wp_ajax_wprp_store_email', array( $this, 'store_email' ) );

		add_action( 'wp_ajax_nopriv_wprp_store_email', array( $this, 'store_email' ) );

	}

	function get_email_storage() {

		$options = wprp_get_settings();
	...
	function store_email() {
		...
		if ( ! wp_verify_nonce( $_POST['wprp_email_manager_nonce'], 
			'wprp_email_manager_nonce' ) ) {
		...
		$storage = apply_filters( 'wprp_email_storage_id', $storage );
		...
		do_action( 'wprp_store_email', $data, $storage );
	...
	function store_email_to_db( $data ) {
		...
		wprp_update_email_store( $emails );

		do_action( 'wprp_store_email_to_db' );

	}

	function store_email_to_mailchimp( $data ) {

		wprp_include( '/libs/MCAPI.class.php' );
		...
		do_action( 'wprp_store_email_to_mailchimp', $data );

	}

	function store_email_to_aweber( $data ) {

		do_action( 'wprp_store_email_to_aweber', $data );

	}

	function store_email_to_campaign_monitor( $data ) {

		do_action( 'wprp_store_email_to_campaign_monitor', $data );
